...
module.exports = function(options) {
  var path = options.path || __dirname + '/../recorded',
    files = [],
    groups = {};
  ...
  if (!files.length) {
    throw new Error('no recorded files :(');
  }
  ...
  lodash.each(files, function(file) {
    ...
    // read & parse
    try {
      var recordedFile = fs.readFileSync(filePath, 'utf-8'),
  ...
  var jadeTemplate = __dirname + '/../views/index.jade',
